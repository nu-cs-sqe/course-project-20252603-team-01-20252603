...
- **TC1: Valid construction** ( :white_check_mark: )
  ...
  - **TC8: New territory starts owned by NullPlayer** ( :x: )
...
- **TC9: Set owner from NullPlayer to HumanPlayer** ( :x: )
...
- **TC10: Change owner from one HumanPlayer to another HumanPlayer** ( :x: )
...
- **TC11: Setting owner to NullPlayer raises exception** ( :x: )
...
- **TC14: Returns false when territory is unclaimed** ( :x: )
...
- **TC17: Place one Infantry on territory with no existing pieces** ( :x: )
...
- **TC18: Place one Infantry on territory with existing Infantry** ( :x: )
